...
//    private int canvasHeight = 1200;
    ...
    public void setViewFieldBoundary(HashMap<Integer,Double> xMap, HashMap<Integer,Double> yMap, boolean df) {
        ...
        //      Math.abs(xMap.get(1)-xMap.get(4)), 
        //      Math.abs(xMap.get(2)-xMap.get(3)), 
        //      Math.abs(xMap.get(2)-xMap.get(4)),
        //      Math.abs(xMap.get(3)-xMap.get(4)) };
        //
        //double[] yy = {Math.abs(yMap.get(1)-yMap.get(2)), 
        //      Math.abs(yMap.get(1)-yMap.get(3)), 
        //      Math.abs(yMap.get(1)-yMap.get(4)), 
        //      Math.abs(yMap.get(2)-yMap.get(3)), 
        //      Math.abs(yMap.get(2)-yMap.get(4)),
        //      Math.abs(yMap.get(3)-yMap.get(4)) };
        //
        ////设置X方向最大跨度为场地宽度
        //this.fieldWidth = (float) maximum(xx);
        //
        ////设置Y方向最大跨度为场地长度
        //this.fieldLength = (float) maximum(yy);
...
//    	Log.e("p4x,p4y", fieldToImage(xMap.get(4), yMap.get(4))[0]+ "," + fieldToImage(xMap.get(4), yMap.get(4))[1]);
    ...
    public void setViewFieldBoundary(ArrayList<GeoPoint> fieldVertex, boolean df) {
        ...
        for (int i = 0; i < fieldVertex.size(); i++) {
            xx[i] = fieldVertex.get(i).getXCoordinate();
        }

        double[] yy = new double[fieldVertex.size()];
        for (int i = 0; i < fieldVertex.size(); i++) {
            yy[i] = fieldVertex.get(i).getYCoordinate();
        }
...
//        Log.e("fWidth,fLength", fieldWidth + "," + fieldLength);
//        Log.e("fp1x,fp1y", fieldVertex.get(0).getXCoordinate() + "," + fieldVertex.get(0).getYCoordinate());
//        Log.e("fp2x,fp2y", fieldVertex.get(1).getXCoordinate() + "," + fieldVertex.get(1).getYCoordinate());
//        Log.e("fp3x,fp3y", fieldVertex.get(2).getXCoordinate() + "," + fieldVertex.get(2).getYCoordinate());
//        Log.e("fp4x,fp4y", fieldVertex.get(3).getXCoordinate() + "," + fieldVertex.get(3).getYCoordinate());
...
//    			field2Image(BPointX, BPointY)[0],field2Image(BPointX, BPointY)[1])[3];
    ...
    public float[] calculateABline(float aX, float aY, float bX, float bY) {
    	...
    	float[] abLine = {startX,startY,endX,endY};
    	return abLine;
    }
...
//        scale = calculateScale(fieldLength, fieldWidth, canvasHeight - canvasMargin, canvasWidth - canvasMargin);
    ...
    private int[] fieldToImage(GeoPoint point) {
        ...
        int xV = canvasWidth / 2 + (int)((point.getXCoordinate() - originX) * scale + 0.5);
        int yV = canvasHeight / 2 - (int)((point.getYCoordinate() - originY) * scale + 0.5);
        int[] viewPoint = {xV, yV};   
        return viewPoint;
    }
